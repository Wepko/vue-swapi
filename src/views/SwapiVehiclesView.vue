...
import { type IVehicleData, ResourcesTypeStarships } from '@/types';
...
const { swapiVehicles, loading, loadingModal } = storeToRefs(swapiStore);
...
const columns = ref<TableColumnsType>([
  ...
  {
    title: 'Название траспортных средств',
    dataIndex: 'name',
    key: 'name',
  },
  {
  ...
  {
    title: 'Класс траспортных средств',
...
const queryData = (params: { page?: number }) => {
  return swapiStore.querySawpiPagination('https://swapi.dev/api/vehicles/', params);
};

const putDataInStore = (data: IVehicleData[]) => {
  (swapiVehicles.value as ISwapiData).results = data;
};
